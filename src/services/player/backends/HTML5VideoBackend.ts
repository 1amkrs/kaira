...
export class HTML5VideoBackend implements IPlayerBackend {
  public readonly type: BackendType = 'html5';
  ...
  private handleError = (): void => {
    if (this.isDestroyed) return;
    const err = this.videoElement?.error?.message || 'Hardware video playback error';
  ...
  public async play(): Promise<void> {
    ...
    try {
      ...
      console.warn('[HTML5VideoBackend] play() exception:', e);
      this.state.status = 'paused';
      this.callbacks?.onStatusChange('paused');
    }
  }
